...
export const countryCodeList: IFlagEntry[] = [
	...
	{ countryName: 'Sudan', alpha2: 'SD', alpha3: 'SDN', numeric: '729' },
...
countryCodeList.sort((a, b) => a.countryName.localeCompare(b.countryName));

export const countryCodeAlpha2Dict: Record<string, IFlagEntry> = {};
countryCodeList.forEach((country) => {
	countryCodeAlpha2Dict[country.alpha2] = country;
});

export const flagUrlFromAlpha2 = (alpha2?: string) => {
	if (alpha2 && countryCodeAlpha2Dict[alpha2]) {
		return `/flags/svg/${alpha2}.svg`;
	} else {
